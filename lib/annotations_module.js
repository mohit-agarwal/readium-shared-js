var EpubAnnotationsModule = function (contentDocumentDOM, bbPageSetView, annotationCSSUrl) {
   ...
   removeHighlight: function(annotationId) {
    ...
    generateRangeCFI : function (selectedRange) {
        ...
        // ADDED by Jonah Dempcy, 4/23/2015

        function createTreeWalker(node) {   // Helper function to create a tree walker for finding text nodes in the DOM
	    ...
	    function findNearestTextNodes(node) { 	    // Helper function to return the nearest text node provided with a given HTML node
	    	if (node.nodeType === 3) return [node];
			var childNodesWalker = createTreeWalker(node);
			var nodes = [];
			while (childNodesWalker.nextNode()) {
				nodes.push(childNodesWalker.currentNode);
			}
			if (nodes.length > 0) {
				return nodes;
			} else {
				var siblingNodesWalker = createTreeWalker($(node)
						.parent().get(0));
				while (siblingNodesWalker.nextNode()) {
					nodes.push(siblingNodesWalker.currentNode);
				}
				if (nodes.length > 0) {
					return nodes;
				} else {
					var parentNodesWalker = createTreeWalker($(node).parent().parent().get(0));
					while (parentNodesWalker.nextNode()) {
						nodes.push(parentNodesWalker.currentNode);
					}
					if (nodes.length > 0) {
						return nodes;
					}
	    ...
	    function findFirstTextNode(node) {
	    	var nodes = findNearestTextNodes(node);
	    	if (nodes.length > 0) {
		    	return nodes[0];	    		
	    	}
	    }
        ...
        endNode = findLastTextNode(endNode);
        // End ADDED section.
